...
- 바이너리 데이터를 전송할 수 없다.
- URL에 데이터가 포함되기 때문에 URL과 데이터를 함께 저장할 수 있다.

=> 특징 정리
- 소량의 데이터를 보낼 때(게시글 상세보기,삭제) 사용
- 바이너리 데이터 전송 불가
...
- 서버에 보내는 데이터는 message-body에 포함된다.
      name=%ED%99%8D%EA%B8%B8%EB%8F%99&age=222
- messge-body에 대한 데이터 정보가 요청 헤더에 포함된다.
      Content-Length: 40
      Content-Type: application/x-www-form-urlencoded
...
- 바이너리 데이터 전송 가능
- 특정 결과를 출력하는 페이지의 링크 정보를 공유해봐야 소용없다.
...
Date: Wed, 24 Aug 2016 02:51:07 GMT
