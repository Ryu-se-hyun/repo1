 ...
 *       1.1 세션이 유효하다면(타임아웃되지 않았다면), 해당 세션 객체를 리턴한다.
 *       1.2 세션이 유효하지 않다면(타임아웃 되거나, 무효화되었다면), [2번]으로 간다.
 *           
 *    2. 웹브라우저가 보낸 요청 헤더에 세션 아이디에 해당하는 쿠키가 없다면, 새로운 세션 객체를 만든다.
 *       2.1 응답할 때 새로 만든 세션 객체의 아이디를 쿠키에 담아 웹브라우저로 보낸다.
 *       2.2 웹브라우저는 서버가 보낸 세션 아이디를 임시로 저장한다.
 *           2.2.1 즉 실행되는 동안만 보관한다.
 *           2.2.2 웹브라우저를 종료(모든 웹브라우저 종료)하면 임시 쿠키가 삭제되기 때문에
 *                 세션 아이디로 삭제된다. 
 *         
 */
package servlet5;
